...
const { applyMoveToProductionWaiting, isProductionWaitingStageRecord } = await import(pathToFileURL(path.join(root,"src/utils/titanWorkflowStatus.js")).href);
...
console.log("waiting", isProductionWaitingStageRecord(record));
console.log("heat", record.heatTreatment, "detail", record.processDetail);
console.log("eq process", eq?.process, "status", eq?.status);
console.log("lots", lots.length, lots[0]?.sourceRecordId);