...
globalThis.localStorage = { getItem:()=>null, setItem:()=>{}, removeItem:()=>{}, get length(){return 0}, key:()=>null };
...
const { getProductByCompanyAndPartNo } = await import(pathToFileURL(path.join(root,"src/utils/productRegistrationSession.js")).href);
...
const prod = stageMasterAdd("products", { company: COMPANY, partNo: "GP0-N-001", name: "GP0\uB0B4\uC6A9\uD488", material: "SCM440", processCategory: "heatTreatment", processDetail: ION, process: ION });
...
const pr = getProductByCompanyAndPartNo(COMPANY, "GP0-N-001");
console.log("master product process", prod.row?.process, prod.row?.processDetail);
console.log("registration product", pr);
console.log("lots", getChargeableLots("ION-01").length);