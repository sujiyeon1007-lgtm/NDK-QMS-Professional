...
export default function EquipmentChargingActions({ buttonState }) {
  ...
  return (
    <div className="qr-equipment-actions" role="group" aria-label="설비 장입 작업">
      {showStart ? (
        <PrimaryButton type="button" disabled={!startEnabled} aria-disabled={!startEnabled}>
          {QR_CHARGING_PAGE_COPY.startLabel}
        </PrimaryButton>
      ) : null}
      {showComplete ? (
        <SecondaryButton type="button" disabled={!completeEnabled} aria-disabled={!completeEnabled}>
