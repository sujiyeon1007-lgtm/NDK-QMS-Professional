...
import {
  ...
  EQUIPMENT_INFO_FIELDS,
  EQUIPMENT_LIST_COLUMNS,
  EQUIPMENT_LOT_COLUMNS,
  EQUIPMENT_MATERIAL_COLUMNS,
  EQUIPMENT_PROCESS_COLUMNS,
  EQUIPMENT_PROFILE_FIELDS,
  EQUIPMENT_QR_FIELDS,
} from "../../config/equipmentDetailSections";
import {
...
/** Sprint 8 · 설비관리 — Domain Master Workspace (Material / Process Master 패턴 재사용) */
export default function EquipmentManagementPage() {
  const screen = getMasterDataScreen("equipment");
  ...
  const summaryKpis = useMemo(() => buildEquipmentMasterSummary(), [refreshKey]);
  const listMeta = useMemo(() => buildEquipmentListMeta(), [refreshKey]);
  ...
  const rowsWithMeta = useMemo(
    () =>
      filteredRows.map((row) => ({
        ...row,
        process: listMeta.get(row.id)?.process ?? (row.equipType?.trim() || "—"),
      })),
  ...
  const detail = useMemo(() => buildEquipmentMasterDetail(selectedRow), [selectedRow, refreshKey]);
      ...
      <div className="company-management-page">
        ...
        <div className="company-management-page__search">
          <input
            ...
            placeholder="설비명 · 설비코드 · 설비종류 · 상태 검색"
            aria-label="설비 검색"
          />
        </div>
          ...
          <div className="domain-master-workspace__detail">
            {selectedRow ? (
              <>
                ...
                <section className="domain-master-detail__summary-card" aria-label="설비 요약">
                  ...
                  <div className="domain-master-summary__metrics">
                    ...
                    <div>
                      ...
                      <strong>{detail.summaryCard.recentLotCount}건</strong>
                    </div>
                    <div>
                      <span className="domain-master-summary__label">가동 상태</span>
                      <strong>{detail.summaryCard.runStatus}</strong>
                    </div>
                  </div>
                </section>

                <nav className="company-detail-tabs" aria-label="설비 상세">
                ...
                <div className="domain-master-detail__body">
                  {activeTab === "profile" ? (
                    <section className="company-detail-section" aria-label="기본정보">
                      <FieldGrid
                        fields={EQUIPMENT_PROFILE_FIELDS}
                        source={selectedRow}
                        variant="profile"
                      />
                    </section>
                  ...
                  {activeTab === "processes" ? (
                    <section className="company-detail-section" aria-label="담당 공정">
                      <DetailTable
                        columns={EQUIPMENT_PROCESS_COLUMNS}
                  ...
                  {activeTab === "materials" ? (
                    <section className="company-detail-section" aria-label="처리 재질">
                      <DetailTable
                        columns={EQUIPMENT_MATERIAL_COLUMNS}
                        rows={detail.materials}
                        emptyMessage="처리 재질 이력이 없습니다."
                      />
                    </section>
                  ) : null}

                  {activeTab === "lots" ? (
                    <section className="company-detail-section" aria-label="최근 작업 LOT">
                      <DetailTable
                        columns={EQUIPMENT_LOT_COLUMNS}
                        rows={detail.lots}
                        emptyMessage="작업 LOT 이 없습니다."
                      />
                    </section>
                  ) : null}
              ...
              <div className="domain-master-detail__empty">
                ...
                <p>좌측 목록에서 설비를 선택하면 상세 정보가 표시됩니다.</p>
              </div>
            )}
          </div>
        </div>
