...
export default function QrMobileWorkMode({
  ...
  const processName = summary.process ?? "";
  const workConditionFields = useMemo(
    () => [...getChargeWorkConditionFields(processName), MOBILE_CHARGE_QTY_FIELD],
    [processName]
  );

  useEffect(() => {
    setConditions((prev) => ({
      ...buildEmptyChargeWorkConditions(processName),
      chargeQty: prev.chargeQty ?? "",
    }));
  }, [processName]);
