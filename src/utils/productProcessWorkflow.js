...
/** Editor-only — keeps blank placeholder steps while user adds/reorders */
export function normalizeProcessWorkflowEditor(steps) {
  if (!Array.isArray(steps)) return [];
  return steps.map((step, index) => ({
    order: index + 1,
    processCategory: String(step?.processCategory ?? "").trim(),
    processDetail: String(step?.processDetail ?? "").trim(),
  }));
}
...
export function buildProductProcessWorkflowSavePatch(productPayload = {}) {
  ...
  const templateId = String(productPayload.processWorkflowTemplateId ?? "").trim();
  if (!firstStep) {
    return {
      processWorkflow: [],
      ...(templateId ? { processWorkflowTemplateId: templateId } : {}),
    };
  }
  return {
...
export function shouldPromptProcessStepOnComplete(record) {
  return getRemainingProcessWorkflowSteps(record).length > 1;
}
